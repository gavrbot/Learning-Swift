...
class ViewController: UIViewController {
    ...
    // ссылка на managed object context
    let context = (UIApplication.shared.delegate as! AppDelegate).persistentContainer.viewContext
        
    // данные для таблицы Core data
    ...
    override func viewDidLoad() {
        ...
        // получаем контакты из Core Data
        fetchContacts()
    }
    
    func fetchContacts() {
        // обрабатываем данные из Core Data чтобы отобразить в tableView
        do {
            self.contacts = try context.fetch(Contact.fetchRequest())
            // обновляем вью в главном потоке
